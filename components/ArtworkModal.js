...
const TABS = ["About", "Details", "Shipping"];
  ...
  const TabContent = () => {
    if (activeTab === "About") return (
      <div className="space-y-4">
        {current.longDescription ? (
          ...
          <p className="text-xs text-white/25 italic">Tidak ada deskripsi.</p>
        )}
        <div className="pt-2 space-y-0">
          <Row label="Year Created" value={current.year} />
          <Row label="Medium"       value={current.medium} />
          <Row label="Category"     value={current.category === "calligraphy" ? "Calligraphy" : "Landscape"} />
        </div>
      </div>
    );
    ...
    if (activeTab === "Details") return (
      <div className="space-y-0">
        <Row label="Medium"        value={current.medium} />
        <Row label="Size"          value={current.size} />
        ...
        <Row label="Rarity"        value="One-of-a-kind Artwork" />
        <Row label="Frame"         value={current.frame} />
        <Row label="Ready to Hang" value={current.readyToHang} />
        <Row label="Authenticity"  value={current.authenticity} />
        <Row label="Packaging"     value={current.packaging} />
    ...
    if (activeTab === "Shipping") return (
      <div className="space-y-0">
        <Row label="Delivery Cost"  value="Free (Harga sudah termasuk ongkos kirim)" />
        <Row label="Delivery Time"  value="3–7 hari kerja (domestik) · 14–30 hari kerja (internasional)" />
        <Row label="Returns"        value="Hubungi kami dalam 3 hari setelah diterima jika ada masalah" />
        <Row label="Handling"       value={current.handling || "Domestik: Dikirim dengan bingkai (framed) & siap pajang. Internasional: Digulung dalam tabung (rolled tube) untuk keamanan."} />
        <Row label="Ships From"     value={current.shipsFrom || "Indonesia"} />
        <Row label="Authenticity"   value={current.authenticity || "Certificate is Included"} />
        <div className="pt-5 text-[10px] text-white/25 tracking-wide">
          Need more information?{" "}
          <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="underline text-maroon/60 hover:text-maroon">
            Contact us via WhatsApp
                    ...
                    <motion.div key={`${current.id}-img-${imageIndex}`}
                      ...
                      {current.status === "collected" && (
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          <span className="text-white font-medium tracking-[0.35em] uppercase text-xs border border-white/30 px-6 py-2.5 bg-black/60 backdrop-blur-sm rounded-sm">Collected</span>
                        </div>
                      )}
                      {totalImages > 1 && (
                ...
                <div className="order-3 lg:w-[300px] shrink-0 flex flex-col max-h-[40dvh] lg:max-h-[80dvh]">
                  ...
                  <div className="shrink-0 space-y-3 pb-3 border-b border-white/8">
                    <div className="flex items-center gap-2">
                      {current.status === "collected" ? (
                        <><div className="w-1.5 h-1.5 rounded-full bg-white/25" /><span className="text-[10px] tracking-[0.2em] uppercase text-white/40 font-medium">Collected</span></>
                      ) : (
                  ...
                  <div className="shrink-0 flex items-center gap-0 border-b border-white/8 mt-3">
                    {TABS.map(tab => (
                      <button key={tab} onClick={() => setActiveTab(tab)}
                        ...
                        {tab === "About" ? "About" : tab === "Details" ? "Details & Dim." : "Shipping"}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="shrink-0 pt-4 space-y-2">
                    <a href={whatsappLink} target="_blank" rel="noopener noreferrer"
                      ...
                      {current.status === "collected" ? "Enquire Similar Works" : "Enquire via WhatsApp"}
                    </a>
                    <p className="text-[10px] text-white/20 text-center tracking-[0.1em]">Every piece is an original, one-of-one work</p>
                  </div>
                </div>
              </motion.div>
            </AnimatePresence>
